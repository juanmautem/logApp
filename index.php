...
<script type="text/javascript">
    ...
    function allUsers(){
        ...
        for (var row of allUsers){
            ...
            cad += "<td>";
                if(row.bActivo = 1)
                    cad += "<button class='btn btn-success btn-sm '> <i class='fas fa-thumbs-up'></i></button>";
                if(row.bActivo = 0)
                    cad += "<button class='btn btn-danger btn-sm '> <i class='fas fa-thumbs-down'></i></button>";
	...
	function login(){
		...
		alert("Estoy en el Login, Usuario: " + usrData['user'] + "; Password : " + usrData['pass']);
        $.ajax({
            method: "POST",
            url: "modLogin.php",
            data: usrData
